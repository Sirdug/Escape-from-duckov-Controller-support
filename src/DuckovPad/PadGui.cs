...
namespace DuckovPad
{
    ...
    internal static class PadGui
    {
        ...
        {
            var family = playStation ? ControllerFamily.PlayStation : ControllerDevice.Family;
            var detectedLabel = ControllerProfile.Label(canonicalName, family);
            if (Glyphs.TryGetValue(canonicalName ?? string.Empty, out var glyph))
            {
                return new GlyphInfo
                {
                    Label = detectedLabel ?? glyph.Xbox,
                    Color = glyph.Color,
                    WidthFactor = detectedLabel == null ? glyph.WidthFactor : Mathf.Max(glyph.WidthFactor, detectedLabel.Length * 0.6f),
                    Arrow = glyph.Arrow
                };
            }
            ...
            return new GlyphInfo
            {
                Label = Pad.FriendlyName(canonicalName) ?? "?",
                Color = Neutral,
                WidthFactor = 2.2f,
                Arrow = 0
            };
