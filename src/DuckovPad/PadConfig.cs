...
namespace DuckovPad
{
    ...
    public class PadConfig
    {
        ...
        public class AimSettings
        {
            ...
            /// "classic" = world-basis twin-stick; "relative" = free cursor.</summary>
            ...
            public float OuterDeadzone = 0.92f;

            /// <summary>Response curve exponent applied to stick magnitude. 1 = linear, higher = finer near centre.</summary>
            public float ResponseCurve = 1.35f;

            /// <summary>How quickly the crosshair settles onto the stick direction, in units/sec.</summary>
            public float TurnResponse = 26f;

            /// <summary>
            /// Ceiling on sweep speed, in degrees/sec. Caps the opening of every turn so a
            /// flick takes a predictable amount of time instead of arriving instantly.
            /// </summary>
            public float MaxTurnSpeed = 1000f;

            /// <summary>
            /// How much a partial stick deflection slows the turn, 0..1. 0 keeps every
            /// deflection at full speed; higher gives more fine control near the centre.
            /// </summary>
            public float PrecisionRange = 0.72f;

            /// <summary>Turn-rate multiplier while aiming down sights.</summary>
            public float AdsPrecision = 0.6f;

            /// <summary>
            /// Reticle distance from the duck, as a fraction of the largest circle that fits
            /// on screen. Constant in every direction — that is what keeps the crosshair
            /// moving at one speed no matter which way it is pointing.
            /// </summary>
            public float Reach = 0.82f;

            /// <summary>Reach multiplier while aiming down sights.</summary>
            public float AdsReachMultiplier = 1.15f;

            /// <summary>
            /// Let the weapon's recoil kick build up and recover. Off pins the crosshair
            /// exactly where the stick puts it, which is steadier but not how the game plays
            /// for a mouse.
            /// </summary>
            public bool PreserveRecoil = true;
        ...
        public class AimAssistSettings
        {
            ...
            /// <summary>Master scale over both halves of assist. 0 = no help at all.</summary>
            public float Strength = 0.75f;

            /// <summary>
            /// How much the turn rate is damped while the crosshair is near a target, 0..1.
            /// This is the half that does not move the aim for you — it just makes an enemy
            /// easier to stop on — so it can be run high without feeling automatic.
            /// </summary>
            ...
            public float StickinessDegrees = 8f;

            /// <summary>Only assist while the trigger or ADS is held.</summary>
            public bool OnlyWhileFiringOrAds = false;

            /// <summary>Sweep faster than this, in degrees/sec, and the pull steps aside so a
            /// deliberate flick past an enemy never drags.</summary>
            public float FlickDegreesPerSecond = 420f;

            /// <summary>How fast the accumulated pull unwinds once no target is in the wedge.</summary>
            public float ReleaseDegreesPerSecond = 110f;
        ...
        public class AimSnapSettings
        {
            ...
            /// <summary>Flicking past the break angle picks the next enemy that way instead of
            /// simply dropping the lock.</summary>
            public bool SwitchTargetOnFlick = true;
            ...
            /// <summary>Outline the locked enemy, using the outline pipeline the game already
            /// runs for the cover you are standing behind.</summary>
            public bool ShowOutline = true;

            /// <summary>Outline colour, as an HTML string. Anything unparseable falls back to white.</summary>
            public string OutlineColor = "#FFFFFF";

            /// <summary>Outline thickness, 0..1, relative to the game's own outline width.</summary>
        ...
        private void Validate()
        {
            ...
            Aim.TurnResponse = Mathf.Clamp(Aim.TurnResponse, 4f, 90f);
            Aim.MaxTurnSpeed = Mathf.Clamp(Aim.MaxTurnSpeed, 120f, 5000f);
            Aim.PrecisionRange = Mathf.Clamp01(Aim.PrecisionRange);
            Aim.AdsPrecision = Mathf.Clamp(Aim.AdsPrecision, 0.2f, 1f);
            Aim.Reach = Mathf.Clamp(Aim.Reach, 0.2f, 1f);
            ...
            AimAssist.MagnetDegreesPerSecond = Mathf.Clamp(AimAssist.MagnetDegreesPerSecond, 0f, 360f);
            AimAssist.MaxPullDegrees = Mathf.Clamp(AimAssist.MaxPullDegrees, 0f, 45f);
            AimAssist.StickinessDegrees = Mathf.Clamp(AimAssist.StickinessDegrees, 0f, 45f);
            AimAssist.MaxAngleDegrees = Mathf.Clamp(AimAssist.MaxAngleDegrees, 1f, 90f);
            AimAssist.MaxDistance = Mathf.Max(1f, AimAssist.MaxDistance);
            AimAssist.FlickDegreesPerSecond = Mathf.Max(0f, AimAssist.FlickDegreesPerSecond);
            ...
            AimSnap.EngageTime = Mathf.Clamp(AimSnap.EngageTime, 0f, 1f);
            AimSnap.ReleaseTime = Mathf.Clamp(AimSnap.ReleaseTime, 0f, 1f);
            AimSnap.LeadDegrees = Mathf.Clamp(AimSnap.LeadDegrees, 0f, 60f);
