...
namespace DuckovPad
{
    ...
    ///
    /// Reuses the game's scroll page and option prefabs, with expandable sections
    /// and both options and bindings together on a single page.
    ///
    ...
    internal sealed class NativeOptionsUi
    {
        ...
        private void Build(OptionsPanel panel)
        {
            ...
            _page = BuildValuePage(holder, sourcePage, "Tab_DuckovPad", panel);
        ...
        private GameObject BuildValuePage(GameObject holder, GameObject sourcePage, string name, OptionsPanel panel)
        {
            ...
            // Graphics mostly contains dropdowns. Borrow sliders from Audio and switches
            // from General too, rather than rebuilding their appearance from scratch.
            AddMissingTemplate<OptionsUIEntry_Slider>(templates, OptionKind.Slider, panel, page.transform);
            AddMissingTemplate<OptionsUIEntry_Toggle>(templates, OptionKind.Toggle, panel, page.transform);
            AddMissingTemplate<OptionsUIEntry_Dropdown>(templates, OptionKind.Choice, panel, page.transform);
            PadSettingsLayout.Build(container, _config, templates);
            foreach (var template in templates.Values)
                if (template != null) UnityEngine.Object.DestroyImmediate(template);
            return page;
        }

        private static void AddMissingTemplate<T>(Dictionary<OptionKind, GameObject> templates,
            OptionKind kind, OptionsPanel panel, Transform parent) where T : Component
        {
            if (templates.ContainsKey(kind)) return;
            foreach (var entry in panel.GetComponentsInChildren<T>(true))
            {
                bool ours = false;
                for (var current = entry.transform; current != null; current = current.parent)
                    if (current.name.StartsWith("Tab_DuckovPad", StringComparison.Ordinal)) { ours = true; break; }
                if (ours) continue;
                templates[kind] = UnityEngine.Object.Instantiate(entry.gameObject, parent, false);
                return;
            }
        }
