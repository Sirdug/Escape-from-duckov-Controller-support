...
namespace DuckovPad
{
    ...
    ///
    /// Easy mode maps the stick to the direction seen on screen and assists aim distance.
    /// Classic world-basis aiming and a free-cursor mode remain available.
    /// </summary>
    internal sealed class AimDriver
    {
        ...
        /// <summary>Smoothed stick direction; screen space in Easy mode.</summary>
        private Vector2 _direction = Vector2.up;
        private float _magnitude;
        private bool _initialized;
        private float _assistDistance;
        ...
        public bool Update(InputManager inputManager, CharacterMainControl character, bool adsHeld, bool firing, float deltaTime)
        {
            ...
            var aim = _config.Aim;
            if (_mode != aim.Mode) { Reset(); _mode = aim.Mode; }
            ...
            bool easy = !string.Equals(aim.Mode, "classic", StringComparison.OrdinalIgnoreCase);
            Vector3 origin = character.transform.position;
            if (!_initialized)
            {
                Vector3 facing = character.transform.forward;
                Vector2 centre = camera.WorldToScreenPoint(origin + Vector3.up * 0.5f);
                Vector2 forward = camera.WorldToScreenPoint(origin + facing * aim.ReticleDistance + Vector3.up * 0.5f);
                _direction = easy ? (forward - centre).normalized : Vector2.up;
                if (_direction.sqrMagnitude < 0.001f) _direction = Vector2.up;
                _initialized = true;
            }
            ...
            if (stick.sqrMagnitude > 0.0001f)
            {
                ...
                {
                    float response = aim.Smoothing * (adsHeld ? aim.AdsPrecision : 1f);
                    float angle = AimMath.SmoothAngle(Mathf.Atan2(_direction.y, _direction.x) * Mathf.Rad2Deg,
                        Mathf.Atan2(target.y, target.x) * Mathf.Rad2Deg, response, deltaTime) * Mathf.Deg2Rad;
            ...
            Vector3 stickDirection = easy ? ScreenDirectionToWorld(camera, origin, _direction) : StickToWorld(camera, _direction);
            Vector3 worldDirection = stickDirection;

            // Easy aiming is directional: thumb pressure no longer pumps the crosshair
            // in and out. Enemy distance is handled by assistance below.
            float distance = easy ? aim.ReticleDistance : Mathf.Lerp(aim.MinReticleDistance, aim.ReticleDistance, _magnitude);
            if (adsHeld) distance *= aim.AdsDistanceMultiplier;

            // Steam Input trackpads and gyro may emit mouse movement alongside a pad.
            // Preserve their world-space cursor until the player deliberately uses the stick.
            Vector2 pointerDelta = ControllerDevice.PointerDelta;
            if (!ControllerDevice.UsePointer) _pointerAim = false;
            if (stick.sqrMagnitude > 0.01f) _pointerAim = false;
            if (ControllerDevice.UsePointer && pointerDelta.sqrMagnitude > 0.001f)
            {
                Vector2 start = _pointerAim ? (Vector2)camera.WorldToScreenPoint(_pointerWorld)
                    : (Vector2)camera.WorldToScreenPoint(origin + stickDirection * distance + Vector3.up * 0.5f);
                Vector2 target = start + pointerDelta * aim.PointerSensitivity;
                target.x = Mathf.Clamp(target.x, 16f, Screen.width - 16f);
                target.y = Mathf.Clamp(target.y, 16f, Screen.height - 16f);
                var ray = camera.ScreenPointToRay(target);
                var plane = new Plane(Vector3.up, origin + Vector3.up * 0.5f);
                if (plane.Raycast(ray, out var enter))
                {
                    _pointerWorld = ray.GetPoint(enter);
                    _pointerAim = true;
                    ClearLock();
                }
            }
            if (_pointerAim)
            {
                ...
                Vector2 pointer = camera.WorldToScreenPoint(_pointerWorld);
                WriteAimPosition(inputManager, pointer);
            ...
            else
            {
                ...
                if (easy && _assistTarget != null)
                    distance = Mathf.Lerp(distance, _assistDistance, Mathf.Clamp01(_assistWeight * 1.5f));
            }

            if (_reach <= 0 || LockedOn) _reach = distance;
            else _reach = Mathf.Lerp(_reach, distance, 1f - Mathf.Exp(-18f * deltaTime));
            distance = _reach;
        ...
        private Vector3 CurrentWorldDirection(Vector3 origin)
        {
            ...
            Vector2 intent = Pad.RightStickRaw;
            if (intent.magnitude <= _config.Aim.Deadzone) intent = _direction;
            return string.Equals(_config.Aim.Mode, "classic", StringComparison.OrdinalIgnoreCase)
                ? StickToWorld(camera, intent) : ScreenDirectionToWorld(camera, origin, intent);
        }

        private static Vector3 ScreenDirectionToWorld(Camera camera, Vector3 origin, Vector2 direction)
        {
            Vector3 centre = origin + Vector3.up * 0.5f;
            Vector2 screen = camera.WorldToScreenPoint(centre);
            var ray = camera.ScreenPointToRay(screen + direction.normalized * 100f);
            var plane = new Plane(Vector3.up, centre);
            if (!plane.Raycast(ray, out var enter)) return StickToWorld(camera, direction);
            Vector3 result = ray.GetPoint(enter) - centre;
            result.y = 0f;
            return result.sqrMagnitude > 0.0001f ? result.normalized : StickToWorld(camera, direction);
        }
        ...
        private bool UpdateLockOn(Vector3 origin, Vector3 stickDirection, out Vector3 direction, out float distance)
        {
            ...
            if (snap.RequireLineOfSight && !TargetFinder.HasLineOfSight(origin, point))
            {
                _lockTarget = null;
                return false;
            }
            ...
            if (snap.BreakOnStickInput && Pad.RightStickRaw.magnitude > 0.7f &&
                Vector3.Angle(CurrentWorldDirection(origin), targetDirection) > snap.BreakAngleDegrees)
            {
                _lockTarget = null;
                return false;
            }
        ...
        private Vector3 ApplyAimAssist(Vector3 origin, Vector3 stickDirection, bool adsHeld, bool firing, float deltaTime)
        {
            ...
            if (result.Found)
            {
                if (_assistTarget != result.Transform) _assistWeight = 0f;
                _assistTarget = result.Transform;
                _assistDirection = result.Direction;
                _assistDistance = result.Distance;
                ...
                desiredWeight = Mathf.Clamp01(settings.Strength * (adsHeld ? 1.25f : 1f)) * taper;
