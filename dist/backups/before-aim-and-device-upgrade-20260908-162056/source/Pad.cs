    ...
    internal static class Pad
    {
        ...
        public static void Poll()
        {
            ...
            if (_gamepad.lastUpdateTime > 0d)
            {
                ...
                if (_gamepad.leftStick.ReadValue().sqrMagnitude > 0.04f ||
                    _gamepad.rightStick.ReadValue().sqrMagnitude > 0.04f ||
        ...
        public static bool AnyButtonDown()
        {
            ...
            return WasPressed("buttonSouth") || WasPressed("buttonEast")
        ...
        public static Vector2 LeftStick(float deadzone, float outerDeadzone, float curve)
            => Shape(_gamepad?.leftStick.ReadValue() ?? Vector2.zero, deadzone, outerDeadzone, curve);

        public static Vector2 RightStick(float deadzone, float outerDeadzone, float curve)
            => Shape(_gamepad?.rightStick.ReadValue() ?? Vector2.zero, deadzone, outerDeadzone, curve);
        ...
        public static Vector2 Shape(Vector2 raw, float deadzone, float outerDeadzone, float curve)
        {
            ...
            float range = Mathf.Max(0.0001f, outerDeadzone - deadzone);
            float normalized = Mathf.Clamp01((magnitude - deadzone) / range);
            if (Math.Abs(curve - 1f) > 0.001f)
                normalized = Mathf.Pow(normalized, curve);
